...
import { InputOrderResponse, OutputOrderResponse } from '../handle-orders/handle-order.model';
...
export class OrdersComponent implements OnInit, OnDestroy {
  // Tab management
    ...
    // Filter state
    filter = {
      invoiceType: '', // tùy chọn tìm kiếm chung (mã, tên KH/NCC)
      startdate: '',
      enddate: '',
    };
    ...
    loadInputOrders(page?: number, size?: number): void {
      ...
      this.inputInvoiceService.loadAllInputInvoice(this.page, this.size).subscribe({
    ...
    loadOutputOrders(page?: number, size?: number): void {
      ...
      this.outputInvoiceService.loadAllOutputInvoice(this.page, this.size).subscribe({
        next: (res: PagedResponse<any>) => {
          const content = Array.isArray(res.content) ? res.content : [];
    ...
    openConfirmComplete(invoice: InputOrderResponse | OutputOrderResponse){
       const modalRef = this.modalService.open(ConfirmModalComponent, {
            size: 'sm',
            position: 'center',
            data: {
              title: 'Xác nhận hoàn tất đơn hàng',
              message: `Bạn muốn hoàn tất đơn hàng: "${invoice.id} - ${invoice.code}"?`,
              confirmText: 'Yes',
              cancelText: 'No',
              danger: true,
          ...
          if (instance && instance.confirm) {
            instance.confirm.subscribe(() => this.completeInvoice(invoice.id));
          }
          
    }
    completeInvoice(id: number): void {
      if(this.activeTab === 'input') {
        this.inputInvoiceService.completeInputInvoice(id).subscribe({
          next: (res: any) => {
        ...
        this.outputInvoiceService.completeOutputInvoice(id).subscribe({
         next: (res: any) => {
            this.toast.showSuccess('Thành công', res.message);
            this.loadOutputOrders(this.page, this.size);
          },
